...
10. Editor detection is a whitelist, not generic parsing: the data directory is the directory directly under `$HOME` taken from the server path (server root = the path before `/out/server-main.js` in the parent process's cmdline, data dir = `dirname(dirname(root))`). `.antigravity-ide-server` → `antigravity`, `.vscodium-server` → `vscodium` (both auto restart); `.vscode-server` / `.vscode-server-insiders` → `vscode` (manual only); anything else or a detection failure → `unknown` (manual only). Layouts: Antigravity `~/.antigravity-ide-server/bin/<ideVersion>-<commit>/`, VSCodium `~/.vscodium-server/bin/<commit>/`, VS Code `~/.vscode-server/bin/<commit>/`. The server root (= `vscode.env.appRoot`) contains `product.json` with a top-level `commit`.
