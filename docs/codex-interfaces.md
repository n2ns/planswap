...
export function parseCommitFromCmdline(cmdline: string): string | undefined; // the 40-hex commit from the bin/<version>-<commit>/ path segment
...
export function planRestart(): ServerPlan;   // locating and triple check of design section 5; any failure → throw Error(localized reason). children are the processes in /proc with ppid === serverPid and pid !== process.pid
...
```
Pid file: `~/.antigravity-ide-server/.<commit>.pid`; the commit is parsed from the `bin/<version>-<commit>/` path segment in `/proc/<ppid>/cmdline` (commit is 40 hex characters). Field 4 of `/proc/<ppid>/stat` is the parent pid (note that the comm field can contain spaces and parentheses, so parse after the last `)`).
