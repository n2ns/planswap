...
- Once enabled the layout matches the Claude page: all accounts list, "Tools" row, add-account section (every non-external row has a pencil button for renaming), with these differences:
  ...
  - **"Takes effect after restart" banner**: when the directory in the state file differs from the directory effective in this window, the top shows "<display name> selected; takes effect after restarting the server" (an unregistered directory shows its path), with the explanation "Restarting disconnects all WSL windows and prompts them to reload; integrated terminals close." and a **Restart server** button. When another window switches, this window shows the banner as well through its state file watcher.
...
| `aiSwitcher.codex.restartServer` | Restart WSL Server | Button of the "takes effect after restart" banner, Command Palette → restart after a modal confirmation as in 10.6 step 5 |
...
3. Modal confirmation: "Switching the Codex account restarts Antigravity's WSL server: all WSL windows disconnect and prompt to reload, all extensions restart, and integrated terminals close. Continue?", button **Continue**.
...
5. Restart the server: first locate and verify it (the parent process `process.ppid` is not 1; its cmdline contains `out/server-main.js` and `--start-server`; the value in the pid file `~/.antigravity-ide-server/.<commit>.pid` equals the server's parent pid); when verification fails, a warning "Cannot restart the WSL server automatically: <reason>" is shown together with the manual method (close all Antigravity windows connected to this distribution, wait at least 5 minutes, then reopen). When it passes, `SIGTERM` is sent to the server, then to every process in `/proc` whose parent is the server (excluding this extension host), one by one (`ESRCH` and `EPERM` are ignored), without waiting.
6. Afterwards every WSL window shows "Cannot reconnect. Please reload the window.", and the user clicks "Reload Window" in each one.
