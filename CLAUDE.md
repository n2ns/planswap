...
- Never call `executeRestart` and never send signals to the Antigravity WSL server process (or its children) as a test: it restarts the server, disconnects every WSL window, closes integrated terminals and ends the current Claude Code session.
