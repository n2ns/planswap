...
- File-system tests (creating, copying or deleting account directories, rc files, state files) must run under a temporary HOME (e.g. `HOME=$(mktemp -d)`). Never write to or delete anything under the real `~`, `~/.claude`, `~/.codex`, `~/.bashrc`, `~/.profile` or `~/.config/planswap`.
