...
export const zhCn: Record<MessageKey, string> = {
  ...
  'share.confirm': '把 {label} 改为与默认账号共享？{dir} 里的会话历史、记忆、设置和其他共享文件夹会移入默认账号并换成链接，登录保持不变。与默认账号不同的文件会以 .from-<名字> 后缀保存在原文件旁边，供手动合并。此操作无法自动撤销。',
  'share.confirmButton': '改为共享',
  'share.confirmCodex': '把 {label} 改为与默认账号共享？{dir} 里的会话、历史、设置、规则、技能和会话数据库会移入默认账号并换成链接，登录和记忆仍按账号独立。与默认账号不同的文件会以 .from-<名字> 后缀保存在原文件旁边，供手动合并。续接由其他 ChatGPT 账号开始的会话可能被服务器拒绝。此操作无法自动撤销。',
