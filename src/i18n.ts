...
export const en = {
  ...
  'share.removeDirDetail':
    "This shared account's directory only holds its login credentials and account caches; its history, settings and other shared data live in the default account and are kept. The login cannot be recovered once deleted.",
  ...
  'tools.syncNotInit': 'The {vendor} part is not initialized; cannot sync shared accounts.',
  'tools.pick.sync': 'Select the vendor whose shared accounts to sync',
  'sync.none': 'No shared {vendor} accounts to sync.',
  'sync.done': 'Synced {count} shared {vendor} account(s) with the default account.',
  'sync.issues': 'Needs attention: {list}',
  'sync.item': '{name}: {notes}',
  'share.addNotes': 'Account {name} was created and shared, but: {notes}',
  ...
  'share.current': 'Switch away from {label} before sharing it.',
  'share.confirm': "Share {label} with the default account? Its history, memory, settings and other shared folders in {dir} are moved into the default account and replaced by links; the login stays. Files that differ from the default account's are kept for manual merging: inside shared folders next to the default file with a .from-<name> suffix, top-level files in the account directory as <file>.independent-backup. This cannot be undone automatically.",
  'share.confirmButton': 'Share',
  'share.confirmCodex': "Share {label} with the default account? Its sessions, history, settings, rules, skills and thread databases in {dir} are moved into the default account and replaced by links; the login and memories stay per account. Files that differ from the default account's are kept for manual merging: inside shared folders next to the default file with a .from-<name> suffix; config.toml, AGENTS.md, hooks.json and the thread databases in the account directory as <file>.independent-backup. Resuming a session started by another ChatGPT account may be rejected by the server. This cannot be undone automatically.",
  'share.done': '{label} is now shared with the default account. {summary}',
  'share.nothingElse': 'Nothing needed manual attention.',
  'share.failed': 'Sharing {label} stopped: {error}',
  ...
  'share.r.refused': 'not shared for safety: {list}',
...
export const zhCn: Record<MessageKey, string> = {
  ...
  'share.removeDirDetail': '这是共享账号，目录里只有它的登录凭据和账号缓存；会话历史、设置等共享数据都在默认账号里，不会被删除。登录凭据删除后无法恢复。',
  ...
  'tools.syncNotInit': '{vendor} 部分未初始化，无法同步共享账号。',
  'tools.pick.sync': '选择要同步共享账号的厂家',
  'sync.none': '没有需要同步的 {vendor} 共享账号。',
  'sync.done': '已将 {count} 个 {vendor} 共享账号与默认账号同步。',
  'sync.issues': '需要处理：{list}',
  'sync.item': '{name}：{notes}',
  'share.addNotes': '账号 {name} 已创建并共享，但：{notes}',
  'share.addLinkFailed': '账号 {name} 已创建，但链接到默认账号失败：{error}',
  'share.addCopyFailed': '账号 {name} 已创建，但复制默认账号的配置失败：{error}',
  'share.current': '请先切换到其他账号，再把 {label} 改为共享。',
  'share.confirm': '把 {label} 改为与默认账号共享？{dir} 里的会话历史、记忆、设置和其他共享文件夹会移入默认账号并换成链接，登录保持不变。与默认账号不同的文件会保留下来供手动合并：共享文件夹里的文件以 .from-<名字> 后缀保存在默认文件旁边，顶层文件则在账号目录里改名为 <文件名>.independent-backup。此操作无法自动撤销。',
  'share.confirmButton': '改为共享',
  'share.confirmCodex': '把 {label} 改为与默认账号共享？{dir} 里的会话、历史、设置、规则、技能和会话数据库会移入默认账号并换成链接，登录和记忆仍按账号独立。与默认账号不同的文件会保留下来供手动合并：共享文件夹里的文件以 .from-<名字> 后缀保存在默认文件旁边，config.toml、AGENTS.md、hooks.json 和会话数据库则在账号目录里改名为 <文件名>.independent-backup。续接由其他 ChatGPT 账号开始的会话可能被服务器拒绝。此操作无法自动撤销。',
  'share.done': '{label} 已改为与默认账号共享。{summary}',
  'share.nothingElse': '没有需要手动处理的内容。',
  'share.failed': '{label} 改为共享时中止：{error}',
